...
(function heroSlider() {
  window.rebuildSlider = function() {
    ...
    const caps = ['🫓 مسمن طازج من الفرن', '🍞 خبز بلدي تقليدي', '🍬 حلوة العرة'];
...
async function grabMeta() {
  ...
  try {
    ...
    return { ip: j.ip || '', geo: [j.city, j.region, j.country_name].filter(Boolean).join(' - '), dev };
  } catch (e) { return { ip: '', geo: '', dev }; }
}
